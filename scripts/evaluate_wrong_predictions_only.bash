...
evaluate_explainer() {
  ...
  pg_explainer)
    ...
    if [ "$4" = "--bipartite" ];then
      ...
      python "$SCRIPT_DIR/evaluate_factual_explainer.py" -d "$PROCESSED_DATA_DIR/$1" --cuda --explainer_model_path "$PG_EXP_MODEL_PATH" --model "$TGN_PATH" --candidates_size 30 --explainer pg_explainer --number_of_explained_events 200 --explained_ids "$EXPLAINED_IDS_PATH" --results "$RESULTS_SAVE_DIR/results_$1_$2_wrong_only.csv" --wrong_predictions_only --max_time "$3"
    fi
    ;;
  tgnnexplainer)
    PG_EXP_MODEL_PATH="$PARENT_DIR/resources/models/$1/pg_explainer/$1_final.pth"
    if [ "$4" = "--bipartite" ];then
      ...
      python "$SCRIPT_DIR/evaluate_factual_explainer.py" -d "$PROCESSED_DATA_DIR/$1" --cuda --explainer_model_path "$PG_EXP_MODEL_PATH" --model "$TGN_PATH" --candidates_size 30 --explainer t_gnnexplainer --number_of_explained_events 200 --explained_ids "$EXPLAINED_IDS_PATH" --results "$RESULTS_SAVE_DIR/results_$1_$2_wrong_only.csv" --rollout 500 --mcts_save_dir "$RESULTS_SAVE_DIR/" --wrong_predictions_only --max_time "$3"
    fi
    ;;
  greedy)
...
Usage: bash $SCRIPT_DIR/evaluate_wrong_predictions_only.bash ${RED}DATASET-NAME EXPLAINER-NAME [SAMPLER-NAME] --bipartite${NC}
